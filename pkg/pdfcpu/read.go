...
import (
	...
	"github.com/garycarr/pdfcpu/pkg/filter"
	"github.com/garycarr/pdfcpu/pkg/log"
	"github.com/garycarr/pdfcpu/pkg/pdfcpu/model"
	"github.com/garycarr/pdfcpu/pkg/pdfcpu/scan"
...
func parseXRefTableEntry(xRefTable *model.XRefTable, s *bufio.Scanner, objNr int, offExtra int64, repairOff, incr int) error {
	if log.ReadEnabled() {
...
func parseXRefTableSubSection(xRefTable *model.XRefTable, s *bufio.Scanner, fields []string, offExtra int64, repairOff, incr int) error {
	if log.ReadEnabled() {
...
func parseTrailerDict(c context.Context, ctx *model.Context, trailerDict types.Dict, offCurXRef *int64, offExtra int64, incr int) (*int64, error) {
	if log.ReadEnabled() {
...
func processTrailer(c context.Context, ctx *model.Context, s *bufio.Scanner, line string, offCurXRef *int64, offExtra int64, incr int) (*int64, error) {
...
func parseXRefSection(c context.Context, ctx *model.Context, s *bufio.Scanner, fields []string, ssCount *int, offCurXRef *int64, offExtra int64, repairOff, incr int) (*int64, error) {
	if log.ReadEnabled() {
...
func parseAndLoad(c context.Context, ctx *model.Context, line string, offset *int64, incr int) error {
	l := line
...
func processObject(c context.Context, ctx *model.Context, line string, offset *int64, incr int) (*bufio.Scanner, error) {
...
func ReadForPageCount(rs io.ReadSeeker, conf *model.Configuration) (*model.Context, error) {
	if log.ReadEnabled() {
		log.Read.Println("ReadForPageCount: begin")
	}
	...
	// Populate xRefTable (same as full read)
	if err = readXRefTable(context.Background(), ctx); err != nil {
		return nil, errors.Wrap(err, "ReadForPageCount: xRefTable failed")
	}

	if err = dereferenceForPageCount(ctx); err != nil {
		return nil, err
	}
	...
	if log.ReadEnabled() {
		log.Read.Println("ReadForPageCount: end")
	}

	return ctx, nil
}

// dereferenceForPageCount loads only the minimal objects needed for page counting.
func dereferenceForPageCount(ctx *model.Context) error {
	if log.ReadEnabled() {
		log.Read.Println("dereferenceForPageCount: begin")
	}

	xRefTable := ctx.XRefTable

	// Handle encryption if present (minimal check)
	if ctx.Encrypt != nil {
		return errors.New("pdfcpu: encrypted PDFs not supported in lightweight page count mode")
	}

	// Dereference only the catalog (root object)
	if ctx.Root == nil {
		return errors.New("pdfcpu: missing root object")
	}

	rootObj, err := dereferencedObject(context.Background(), ctx, ctx.Root.ObjectNumber.Value())
	if err != nil {
		return err
	}
	...
	if !ok {
		return errors.New("pdfcpu: root object is not a dictionary")
	}
	xRefTable.RootDict = rootDict

	// Dereference the Pages object
	pagesRef := rootDict.IndirectRefEntry("Pages")
	if pagesRef == nil {
		return errors.New("pdfcpu: missing Pages entry in catalog")
	}

	pagesObj, err := dereferencedObject(context.Background(), ctx, pagesRef.ObjectNumber.Value())
	if err != nil {
		return err
	}
	...
	if !ok {
		return errors.New("pdfcpu: Pages object is not a dictionary")
	}

	// Extract page count from Pages dictionary
	pageCount := pagesDict.IntEntry("Count")
	if pageCount == nil {
		return errors.New("pdfcpu: missing Count entry in Pages dictionary")
	}

	xRefTable.PageCount = *pageCount

	if log.ReadEnabled() {
		log.Read.Println("dereferenceForPageCount: end")
	}
